...
d |> 
  ...
  labs(title = "Nombre de maintenance 2 de vérins piqueur DPA", y = "")
```
...
survStan::plotHistoAgeWeibull(
   ...
   group_col = "group",
   age_max = 1000,
   tt = "Histogramme des ages des vérins"
  )
```

```{r estimDurVie, include=TRUE}
post <- survStan::getMeanLifePosterior(
  data_vie, group_col = "group", time_col = "dur_vie",censor_col = "C", maxLife = 1000, age_target = 500
)
survStan::plotPostLife(post, xScale = c(0, 1000), t = "Distribution des durées de vie moyennes des  vérins piqueurs réparés") +
  scale_x_continuous(breaks = seq(0, 1000, 100)) 
```

```{r plotDurVie1, include=TRUE}
