...
# d_raw_long
d_raw_lg <- d_raw |> 
  tidyr::pivot_longer(cols = starts_with(c("pan", "de", "con")),
                      names_to = "origine",
...
d_raw_lg |> 
  ...
  labs(title = "Distribution des Anomalies sur cuve",
       subtitle = glue("{n_dpa} DPA / {n_anomalie_cuve} anomalies"),
       x = "",
       y = ""
       )
```
...
d_raw_lg |> 
  ...
  labs(title = "Distribution des Réparations Mille",
       subtitle = glue("{n_dpa} DPA / {n_anomalie_mille} réparations"),
       x = "",
       y = ""
       )
```
...
```{r}
#| label: groups_conclusion
#| include: true
d_raw_lg |> 
  filter(stringr::str_starts(origine, "con")) |> 
  group_by(type_anomalie)  |> 
  summarise(n = n()/n_dpa) |> 
  ungroup()  |> 
  arrange(desc(n)) |> 
  gt::gt(
    caption = glue("Interventions Mille sur {n_dpa} DPA")
  ) |> 
  gt::opt_stylize() |> 
  gt::fmt_percent(columns = n, decimals = 0) |> 
  gt::cols_label(
    type_anomalie = "Intervention Mille",
    n = "%"
  )
```
