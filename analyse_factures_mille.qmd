...
ddd |>
  ...
  geom_hline(yintercept = cout_moyen_mois, lty = 2) +
...
#| include: true
plot_pareto_cout("verin_piqueur", poste, "Décomposition des Coûts  Vérins Piqueurs")
```
...
```{r}
#| label: nb_dpa_transport
#| include: true
ddd |> 
  filter(poste == "prise_en_charge_transport") |> 
  group_by(mois) |> 
  summarise(nb_dpa = sum(nombre)) |> 
  ggplot(aes(x = mois, y = nb_dpa)) +
  geom_col(fill = pal_cat["verin_piqueur"]) +
  geom_text(aes(label = nb_dpa), vjust = -0.5) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%y") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Nombre de DPA transportés par mois",
       subtitle = date_range_title,
       x = "",
       y = "Nombre de DPA")
```
