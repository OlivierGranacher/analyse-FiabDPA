...
dmodel <- dcp |> left_join(dcuves_essai, join_by(cod_cuve)) |> 
  ...
  mutate(groupe_col =  factor(groupe, levels = c(0, 1), labels = c("ref", "essai"))) |> 
...
#| include: true
# modele lmer
mod_lmer_ntea <- lme4::glmer(formula = "ntea ~ groupe_col*period_essai_col + (1 | cod_cuve) + (1|dat)", data = dmodel, family = poisson(link = "log"))
...
#| label: effet_tps_dpa
#| include: true
mod_lmer_tps_dpa <- lme4::lmer(formula = "tps_dpa_moy ~ groupe_col*period_essai_col + (1 | cod_cuve) + (1|dat)", data = dmodel)
...
#| label: effet_nb_piq
#| include: true
mod_lmer_nbpiq <- lme4::lmer(formula = "nbpiq ~ groupe_col*period_essai_col + (1 | cod_cuve) + (1|dat)", data = dmodel)
...
#| label: effet_hbd
#| include: true
mod_lmer_hbd <- lme4::lmer(formula = "hbd ~ groupe_col*period_essai_col + (1 | cod_cuve) + (1|dat)", data = dmodel)
