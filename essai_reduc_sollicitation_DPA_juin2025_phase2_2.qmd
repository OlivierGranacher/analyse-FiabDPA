...
```{r}
#| label: export_data_model
#| eval: false
# Export de dmodel
dmodel |> 
  readr::write_rds(file = "data/data_soll_dpa.rds")
```
...
dmodel |> 
  ggplot(aes(x = sea, fill= groupe_)) +
  geom_histogram(bins = 100, position = "identity", alpha = 0.5, color = "black") +
  customColors::scale_fill_potline(name = "", reverse = T) +
  scale_x_log10() +
  facet_wrap(~ groupe_, scales = "free_y", ncol = 1) +
  labs(title = "Distribution de la Surtension d'Effet d'Anode", x = "SEA (mV)", y = "Nombre de valeurs") +
  theme(legend.position = "bottom")
# export des valeurs de SEA au format csv
write.csv(dmodel |> select(cod_cuve, dat, period_essai, groupe, sea), "data/sea_essai_soll_DPA.csv", row.names = F)
```
...
# Fit a hurdle model with interaction and random effect for cod_cuve and dat

model_tweedie_sea <- glmmTMB(
  sea ~ groupe_ * period_essai_ + (1 | cod_cuve) + (1 | dat),
...
#summary(model_sea)
sjPlot::tab_model(model_tweedie_sea, title = "Modèle lmer sea", digits = 1)
